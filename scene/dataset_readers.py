...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, semantic_folder, depth_folder):
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):
        ...
        semantic_map_path = os.path.join(semantic_folder, image_name) + '.pt' 
        semantic_map_name = os.path.basename(semantic_map_path).split(".")[0]
        semantic_map = torch.load(semantic_map_path, weights_only=True)

        depth_map_path = os.path.join(depth_folder, image_name) + '.npy' 
        depth_map = torch.from_numpy(np.load(depth_map_path))
...
def readColmapSceneInfo(path, images, semantics, depths, eval, llffhold=100):
    try:
    ...
    semantic_dir = "segmentation_logits" if semantics == None else semantics
    depth_dir = "pred_depths" if depths == None else depths
    
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir), semantic_folder=os.path.join(path, semantic_dir), depth_folder=os.path.join(path, depth_dir))
