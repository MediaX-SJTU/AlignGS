...
def PearsonDepthLoss(depth_pred, depth_gt):
    """take the mim
    Args:
...
def cosine_similarity(a, b):
    dot_product = torch.sum(a * b, dim=0)  # 点积
    norm_a = torch.norm(a, dim=0)  # A的L2范数
    norm_b = torch.norm(b, dim=0)  # B的L2范数
    return dot_product / (norm_a * norm_b + 1e-8)  # 防止除零


def edge_aware_normal_loss(normals, semantic_logits):

    semantic_mask = torch.argmax(semantic_logits, dim=0)
    
    right_diff = semantic_mask[:, :-1] != semantic_mask[:, 1:]    
    down_diff = semantic_mask[:-1, :] != semantic_mask[1:, :]

    right_mask = right_diff
    down_mask = down_diff

    right_cosine = cosine_similarity(normals[:, :, :-1], normals[:, :, 1:])
    down_cosine = cosine_similarity(normals[:, :-1, :], normals[:, 1:, :]) 

    # right_mask = (right_cosine > 0.9) & right_mask
    # down_mask = (down_cosine > 0.9) & down_mask

    num_right_pairs = right_mask.sum().item()
    num_down_pairs = down_mask.sum().item()

    # print((right_mask * right_cosine).sum() / num_right_pairs)

    normal_loss_right = right_mask * (1 - torch.sigmoid(100 * (1 - right_cosine)))
    normal_loss_down = down_mask * (1 - torch.sigmoid(100 * (1 - down_cosine)))

    normal_loss = (normal_loss_right.sum() + normal_loss_down.sum()) / (num_right_pairs + num_down_pairs)
