...
def calculate_edit_mask(gaussian_features: torch.Tensor, target_labels: list, opt: str, score_threshold: float = None):

    probs = torch.softmax(gaussian_features, dim=-1)  # (N_points, 150)

    pred_labels = torch.argmax(probs, dim=-1)  # (N_points,)
    mask = torch.isin(pred_labels, torch.tensor(target_labels).to(pred_labels.device))

    if score_threshold is not None:
        scores = probs[:, target_labels].sum(dim=-1)  # (N_points,)
        if opt == "deletion":
            mask = torch.bitwise_or(mask, scores >= score_threshold)
        else:
            mask = torch.logical_and(mask, scores >= score_threshold)
        
    return mask.unsqueeze(1)


def render_edit(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, edit_dict : dict,  
    ...
    target_label = edit_dict["labels"]
    op_dict = edit_dict["operations"]
    threshold = edit_dict["threshold"]

    # edtiing
    if "deletion" in op_dict:
        mask = calculate_edit_mask(semantic_feature[:, 0, :], target_label, "deletion", threshold)
        opacity.masked_fill_(mask, 0)
        # print(scores) # tensor(1., device='cuda:0') tensor(0., device='cuda:0')
    if "extraction" in op_dict:
        mask = calculate_edit_mask(semantic_feature[:, 0, :], target_label, "extraction", threshold)
        opacity.masked_fill_(~mask, 0)
    if "color_func" in op_dict:
        mask = calculate_edit_mask(semantic_feature[:, 0, :], target_label, "color_func", threshold)
        shs[:, 0, :] = shs[:, 0, :] * ~mask + op_dict["color_func"](shs[:, 0, :]) * mask
