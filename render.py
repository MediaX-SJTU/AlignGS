...
from gaussian_renderer import render, render_edit, render_feature
import torchvision
...
def render_segmentation(segmentation_logits, dataset):
    ...
    segmentation_values = colormap_array[(segmentation_logits + 1).cpu().numpy()]
...
def parse_edit_config_and_text_encoding(edit_config):
    ...
    if edit_config is not None:
        ...
        for t in targets:
            if t not in objects:
                raise ValueError(f"Target '{t}' is not in the defined object list")
            edit_dict["targets"].append(t)
            edit_dict["labels"].append(objects.index(t))
        ...
        edit_dict["threshold"] = edit_config["edit"]["threshold"]
...
def render_set(model_path, name, iteration, views, scene, gaussians, pipeline, background, 
               edit_config, max_depth=5.0, volume=None, use_depth_filter=False):
    if edit_config != "no editing":
        edit_dict = parse_edit_config_and_text_encoding(edit_config)
        target = edit_dict["targets"]
        ...
        edit_feature_map_path = os.path.join(model_path, name, "ours_{}_{}_{}".format(iteration, next(iter(edit_dict["operations"])), target), "renders_feature_map")
        ...
        for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
            gt, _ = view.get_image()
            out = render_edit(view, gaussians, pipeline, background, edit_dict) 

            torchvision.utils.save_image(out["render"], os.path.join(edit_render_path, view.image_name + ".png")) 
            torchvision.utils.save_image(gt, os.path.join(edit_gts_path, view.image_name + ".png"))

            gt_feature_map = view.semantic_map.cuda()
            gt_mask = torch.argmax(gt_feature_map, dim=0)
            gt_feature_map_vis = render_segmentation(gt_mask, "ade20k")
            gt_feature_map = gt_feature_map.cpu().numpy().astype(np.float16)
            gt_feature_map_vis.save(os.path.join(edit_gt_feature_map_path, view.image_name + ".png"))
            # torch.save(torch.tensor(gt_feature_map).half(), os.path.join(edit_gt_feature_map_path, view.image_name + ".pt"))

            render_feature_map = out["feature_map"]
            render_mask = torch.argmax(render_feature_map, dim=0)
            render_feature_map_vis = render_segmentation(render_mask, "ade20k")
            render_feature_map = render_feature_map.cpu().numpy().astype(np.float16) 
            render_feature_map_vis.save(os.path.join(edit_feature_map_path, view.image_name + ".png"))
            # torch.save(torch.tensor(render_feature_map).half(), os.path.join(edit_feature_map_path, view.image_name + ".pt")) 
        ...
        render_feature_map_path = os.path.join(model_path, name, "ours_{}".format(iteration), "renders_feature_map")
        ...
        makedirs(render_feature_map_path, exist_ok=True)
        ...
        for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
            ...
            gt_feature_map_vis = render_segmentation(gt_mask, "ade20k") 
            gt_feature_map = gt_feature_map.cpu().numpy().astype(np.float16)
            gt_feature_map_vis.save(os.path.join(gt_feature_map_path, view.image_name + ".png"))
            # torch.save(torch.tensor(gt_feature_map).half(), os.path.join(gt_feature_map_path, view.image_name + ".pt"))

            render_feature_map = render_feature(view, gaussians, pipeline, background)
            render_mask = torch.argmax(render_feature_map, dim=0)
            render_feature_map_vis = render_segmentation(render_mask, "ade20k")
            render_feature_map = render_feature_map.cpu().numpy().astype(np.float16)
            render_feature_map_vis.save(os.path.join(render_feature_map_path, view.image_name + ".png"))
            # torch.save(torch.tensor(render_feature_map).half(), os.path.join(render_feature_map_path, view.image_name + ".pt"))      
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool,
                edit_config: str, max_depth : float, voxel_size : float, num_cluster: int, use_depth_filter : bool):
    with torch.no_grad():
        ...
        if not skip_train:
            render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), scene, gaussians, pipeline, background, 
                       edit_config, max_depth=max_depth, volume=volume, use_depth_filter=use_depth_filter)
            if edit_config == "no editing":
        ...
        if not skip_test:
            render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), scene, gaussians, pipeline, background, edit_config)
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.edit_config, args.max_depth, args.voxel_size, args.num_cluster, args.use_depth_filter)